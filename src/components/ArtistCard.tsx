    ...
    return (
      <div key={i} style={{width:"100%",height:"h-auto"}} className='relative group max-w-[40%] md:max-w-[14rem] flex-wrap'>
              <Link to={`/artists/${track?.artists[0]?.adamid}`}>
                  <LazyLoadImage afterLoad={()=>setContentLoaded(true)} effect='blur' src={track?.images?.background} alt={track?.title} className='shaped--border__radius group-hover:rounded-3xl transition-all' />
              </Link>
