...
const Seekbar:React.FC<SeekbarProps> = ({ value, min, max, onInput,playerExpanded}) => {
  ...
  if(!playerExpanded){
    ...
    <div className="hidden sm:flex flex-row items-center">
      ...
      <p className="text-white text-sm opacity-80 min-w-[2.25rem]">{value === 0 ? '0:00' : getTime(value)}</p>
      <input
        ref={progressBarRef}
        type="range"
        step="any"
      ...
      />
      <p className="text-white text-sm opacity-80 min-w-[2.25rem]">{max === 0 ? '0:00' : getTime(max)}</p>
